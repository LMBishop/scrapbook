...
func NewMux(cfg *config.MainConfig, siteIndex *index.SiteIndex, authenticator *auth.Authenticator) *http.ServeMux {
	...
	mux.HandleFunc("GET /site/{site}/config", middleware.MustAuthenticate(authenticator, handler.GetConfig(siteIndex)))
	mux.HandleFunc("POST /site/{site}/config", middleware.MustAuthenticate(authenticator, handler.PostConfig(cfg, siteIndex)))
